...
from models.archs.DCNv2.dcn_v2 import DCN_sep_pre_multi_offset as DynAgg
from .blocks import PatchEmbed, PatchUnEmbed, ResidualGroup, Channel_Spatial_Attention_Module, CAB 
...
class RestorationNet(nn.Module):
    ...
    def re_init_dcn_offset(self):
        ...
        self.dyn_agg_restore.small_dyn_agg_down.conv_offset_mask.weight.data.zero_()
        self.dyn_agg_restore.small_dyn_agg_down.conv_offset_mask.bias.data.zero_()
        self.dyn_agg_restore.medium_dyn_agg_down.conv_offset_mask.weight.data.zero_()
        self.dyn_agg_restore.medium_dyn_agg_down.conv_offset_mask.bias.data.zero_()
        self.dyn_agg_restore.large_dyn_agg_down.conv_offset_mask.weight.data.zero_()
        self.dyn_agg_restore.large_dyn_agg_down.conv_offset_mask.bias.data.zero_()

        self.dyn_agg_restore.small_dyn_agg_up.conv_offset_mask.weight.data.zero_()
        self.dyn_agg_restore.small_dyn_agg_up.conv_offset_mask.bias.data.zero_()
        self.dyn_agg_restore.medium_dyn_agg_up.conv_offset_mask.weight.data.zero_()
        self.dyn_agg_restore.medium_dyn_agg_up.conv_offset_mask.bias.data.zero_()
        self.dyn_agg_restore.large_dyn_agg_up.conv_offset_mask.weight.data.zero_()
        self.dyn_agg_restore.large_dyn_agg_up.conv_offset_mask.bias.data.zero_()
...
class DynamicAggregationRestoration(nn.Module):
    ...
    def __init__(self, ngf=64, n_blocks=16, groups=8):
        ...
        self.csam_samll = Channel_Spatial_Attention_Module()#64
        self.lam_samll = CAB(ngf,False)#64
        self.small_2 = nn.Sequential(nn.Conv2d(ngf*2, ngf, kernel_size=3, stride=1, padding=1),nn.LeakyReLU(0.1, True))
        ...
        self.csam_medium = Channel_Spatial_Attention_Module()#64
        self.lam_medium = CAB(ngf,False)#64
        self.medium_2 = nn.Sequential(nn.Conv2d(ngf*2, ngf, kernel_size=3, stride=1, padding=1),nn.LeakyReLU(0.1, True))
        ...
        self.csam_large = Channel_Spatial_Attention_Module()#64
        self.lam_large = CAB(ngf,False)#64
        self.large_2 = nn.Sequential(nn.Conv2d(ngf*2, ngf, kernel_size=3, stride=1, padding=1),nn.LeakyReLU(0.1, True))
    ...
    def forward(self, x, pre_offset, img_ref_feat, mask):
        ...
        # small down scale   
        relu3_swapped_feat_down = self.lrelu(self.small_dyn_agg_down([img_ref_feat['relu3_1_down'], relu3_offset],pre_offset['relu3_1_down'])) #[9,256,40,40]
        # small up scale
        relu3_swapped_feat_up = self.lrelu(self.small_dyn_agg_up([img_ref_feat['relu3_1_up'], relu3_offset], pre_offset['relu3_1_up'])) #[9,256,40,40]
        # small swapped_feat
        ...
        h = self.head_small(h)
        h1 = self.lam_samll(h)
        h2 = self.csam_samll(h)
        h = self.small_2(torch.cat([h1, h2], 1))
        ...
        # medium down scale
        relu2_swapped_feat_down = self.lrelu(self.medium_dyn_agg_down([img_ref_feat['relu2_1_down'], relu2_offset],pre_offset['relu2_1_down'])) #[9,128,80,80]
        # medium up scale 
        relu2_swapped_feat_up = self.lrelu(self.medium_dyn_agg_up([img_ref_feat['relu2_1_up'], relu2_offset], pre_offset['relu2_1_up'])) #[9,256,40,40]
        # x2 swapped_feat
        ...
        h1 = self.lam_medium(h)
        h2 = self.csam_medium(h)
        h = self.medium_2(torch.cat([h1, h2], 1))
        ...
        # large down scale
        relu1_swapped_feat_down = self.lrelu(self.large_dyn_agg_down([img_ref_feat['relu1_1_down'], relu1_offset],pre_offset['relu1_1_down'])) #[9,64,160,160]
        # large up scale
        relu1_swapped_feat_up = self.lrelu(self.large_dyn_agg_up([img_ref_feat['relu1_1_up'], relu1_offset], pre_offset['relu1_1_up'])) #[9,256,40,40]
        # large swapped_feat
        ...
        h = self.head_large(h)
        h1 = self.lam_large(h)
        h2 = self.csam_large(h)
        h = self.large_2(torch.cat([h1, h2], 1))
